...
DrawValueWorld:
    ...
    tax                               ;
    adc #5                            ; offset it based on how much we're writing
    ...
    sta VRAM_Buffer1+1,x              ;
    lda #2                            ; we're writing 1 number and possibly an indicator
    sta VRAM_Buffer1+2,x              ;
    lda Settables,y                   ; get the value of the settable item
    pha                               ; push to the stack so we can use it later
    adc #1                            ; increment it, since we display 1-based numbers
    cmp #$0a                          ; have we selected a 2J world?
    bcc @DrawWorldNum                 ; no, draw world number as-is
    sbc #$09                          ; otherwise subtract 9 for display
@DrawWorldNum:                        ;
    sta VRAM_Buffer1+3,x              ; store the number to be drawn
    pla                               ; get the value of the settable item again
    cmp #$09                          ; did we select a 2J world?
    bcc @NoStarIcon                   ; no, put blank space instead of star icon
    lda #$9f                          ; otherwise print a star to indicate 2J worlds
    bne @StoreStarIcon                ; branch ahead
@NoStarIcon:                          ;
    lda #$24                          ; blank space ID
@StoreStarIcon:                       ;
    sta VRAM_Buffer1+4,x              ; store star icon/blank space here
